...
exports.run = async (client, message, args, database) => {
  ...
  .setColor(0x4BB543)
  .setDescription(`${message.author}, o membro ${member.user.username} foi banido com sucesso !`)
  message.channel.send(A_4);
  member.ban(reason);

  let A_5 =  new Discord.RichEmbed()
  .addField('Membro banido', `${member.user.username}#${member.user.discriminator}`)
